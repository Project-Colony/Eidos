...
//! list only. Directory/member conflicts are handled by `eidos-conflicts`;
//! game-specific activation is defined in [`archives`].
...
pub fn enable_bsa_invalidation(ini_dir: &Path, data_dir: &Path, game_id: &str) -> io::Result<()> {
    ...
    if let Some(inv) = pre_sse_invalidation(game_id) {
        ...
        let existing = fs::symlink_metadata(&bsa);
        let replace = match existing {
            Err(e) if e.kind() == io::ErrorKind::NotFound => true,
            Err(e) => return Err(e),
            // Upgrade only the exact bytes generated by earlier Eidos versions.
            // Other archives and symlinks may belong to the user.
            Ok(meta) if meta.is_file() && meta.len() == 83 => {
                fs::read(&bsa)? == legacy_dummy_bsa_bytes(inv.bsa_version)
            }
            Ok(_) => false,
        };
        if replace {
            fs::create_dir_all(data_dir)?;
            write_atomic(&bsa, &dummy_bsa_bytes(inv.bsa_version))?;
        }
...
pub fn register_morrowind_archives(ini: &Path, mod_bsas: &[String]) -> io::Result<()> {
    let (mut text, latin1) = read_ini_text(ini)?;
    ...
    write_ini_text(ini, &text, latin1)
}
...
/// A dummy invalidation BSA: a minimal valid archive
...
fn dummy_bsa_bytes(version: u32) -> Vec<u8> {
    ...
    // File-record block: a length-prefixed, terminated folder name, then a file
    // record (16 bytes). The prefix counts the terminator even for an empty name.
    out.push(1);
    out.push(0);
    out.extend_from_slice(&gen_hash(FILE_NAME).to_le_bytes());
    out.extend_from_slice(&0u32.to_le_bytes()); // size
    out.extend_from_slice(&(70u32 + total_file_name_len).to_le_bytes()); // zero-byte data at EOF

    // The file name + null. The file record already carries the size.
    out.extend_from_slice(FILE_NAME.as_bytes());
    out.push(0);
    out
}

/// Exact historical encoding, used only to recognize archives safe to migrate.
fn legacy_dummy_bsa_bytes(version: u32) -> Vec<u8> {
    let mut bytes = dummy_bsa_bytes(version);
    bytes.remove(52); // The old writer omitted the folder-name length prefix.
    bytes[65..69].copy_from_slice(&82u32.to_le_bytes());
...
pub fn set_ini_key(path: &Path, section: &str, key: &str, value: &str) -> io::Result<()> {
    ...
    write_ini_text(path, &out, latin1)
}

fn write_ini_text(path: &Path, out: &str, latin1: bool) -> io::Result<()> {
    if latin1 {
...
mod tests {
    ...
    #[test]
    fn morrowind_archive_registration_preserves_legacy_ini_bytes() {
        let path = tmp();
        let original = b"[General]\nGreeting=Fran\xe7ais\n[Archives]\nArchive 0=Morrowind.bsa\n";
        fs::write(&path, original).unwrap();
        register_morrowind_archives(&path, &["Mod.bsa".into()]).unwrap();
        let result = fs::read(&path).unwrap();
        let greeting = b"Greeting=Fran\xe7ais\n";
        assert!(result.windows(greeting.len()).any(|w| w == greeting));
        assert!(result
            .windows(b"Archive 1=Mod.bsa".len())
            .any(|w| w == b"Archive 1=Mod.bsa"));
    ...
    fn dummy_bsa_has_a_valid_header() {
        ...
        assert_eq!(&b[52..54], &[1, 0]);
        assert_eq!(u32::from_le_bytes(b[66..70].try_into().unwrap()), 80);
        assert!(b.ends_with(b"dummy.dds\0"));
    }

    #[test]
    fn generated_invalidation_archives_have_readable_members() {
        for game in ["oblivion", "fallout3", "falloutnv", "skyrim"] {
            let docs = tmp_dir();
            let data = tmp_dir();
            enable_bsa_invalidation(&docs, &data, game).unwrap();
            let path = data.join(pre_sse_invalidation(game).unwrap().bsa_name);
            assert_eq!(
                eidos_conflicts::read_archive_members(&path).unwrap(),
                ["dummy.dds"]
            );
    ...
    #[test]
    fn invalidation_upgrades_only_its_exact_legacy_archive() {
        for game in ["oblivion", "falloutnv"] {
            let docs = tmp_dir();
            let data = tmp_dir();
            let inv = pre_sse_invalidation(game).unwrap();
            let path = data.join(inv.bsa_name);
            let legacy = legacy_dummy_bsa_bytes(inv.bsa_version);
            assert_eq!(legacy.len(), 83);
            ...
            let unrelated = vec![42; 83];
            fs::write(&path, &unrelated).unwrap();
            enable_bsa_invalidation(&docs, &data, game).unwrap();
            assert_eq!(fs::read(&path).unwrap(), unrelated);
            fs::remove_file(&path).unwrap();
            std::os::unix::fs::symlink("user-archive.bsa", &path).unwrap();
            enable_bsa_invalidation(&docs, &data, game).unwrap();
            assert!(!data.join("user-archive.bsa").exists());
